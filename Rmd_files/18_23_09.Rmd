...
# subset
tweets <- filter(tweets, day > 18)
tweets <- filter(tweets, day < 23)
```
...
orig %>%
  ...
  geom_col(fill = c("darkorchid2","limegreen", "gold","red2","black","deepskyblue1"), alpha = .8) +
  coord_flip()
```
...
news %>%
  ...
  xlab("") +
  ylab("%Anteil der Tweets") +
  geom_col(fill = c("darkorchid2","limegreen", "gold","black","red2", "deepskyblue1"), alpha = .8) +
  coord_flip()
```
...
ignore <- paste(c("cdu","bündnis", "spd","fdp","gruene","grüne","grünen", "linke","dielinke","afd","gen","merkel","lindner","schulz","csu","union"), collapse = "|")
all <- gsub(ignore,"", all)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ignore <- c("cdu","spd","fdp","linke","linken","dielinke","gruene", "grüne","grünen", "merkel","schulz","lindner","union")
...
news.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("black","gold","darkorchid2", "deepskyblue1","limegreen","red2"), alpha = .8) 
...
user.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("deepskyblue1","black", "red2","gold","limegreen","darkorchid2"),alpha=.8) 
```
