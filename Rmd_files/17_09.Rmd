...
---
Auch wenn man durchaus den Eindruck gewinnen kann, dass der Wahlkampf bisher eher mit angezogener Handbremse geführt wurde, nimmt er zumindest in den Medien, nun ca. anderthalb Wochen vor der Bundestagswahl, an Intensität zu. Höchste Zeit also, um ein Stimmungsbild in den sozialen Medien zu den zur Wahl stehenden Parteien einzufangen. Aus diesem Grund haben wir alle Twitter-Tweets der letzten Tage (10.09.2017 - 15.09.2017) zu AFD, Bündnis90/Die Grünen, CDU/CSU, Die Linke und SPD erhoben und ausgewertet (Wir haben Tweets gesammelt, die einen oder mehrere Nennungen der Parteien enhalten).  

Im Focus unserer Analyse stehen sowohl die Quantität der Tweets zu den einzelnen Parteien als auch deren jeweilige Tonalität. Wird also eine Partei häufig bei Twitter kommentiert? Und steht der Tweet eher in einem positiven oder negativen Kontext? Und welche Wörter werden am häufigsten in den Tweets genannt. Ebenso erstellen wir einen Index über die Tonalität der Tweets und vergleichen somit die Stimmung zu den Parteien im Social Web. Außerdem unterteilen wir die Tweets dahingehend, ob sie (a) von einem privaten Account, (b) von einem Account eines privaten Nachrichtendienstes oder (c) von einem öffentlich-rechtlichen Nachrichtendienst gesendet wurde.

Die Untersuchung führen wir mit den Methoden des Text Minings wie zum Beispiel der Sentiment Analyse mithilfe der Statistik Software R durch. 
...
# load the dataframe
tweets <- read_csv("~/GitHub/politsentiment/tweets")
...
```{r, include=FALSE}
# subset
tweets <- tweets[tweets$date > as.Date("2017-09-11"),]
tweets <- tweets[tweets$date < as.Date("2017-09-16"),]
```
...
my_stopwords <- c(stopwords("german"), 'rt','https','t','amp','dass','beim','co','via',"grüne","gruene")
...
col <- c("deepskyblue1", "black", "gold","limegreen","darkorchid2", "red2")

ggplot(tweets, aes(date, fill = partei)) + 
    geom_histogram(alpha = .7, adjust = .25, binwidth = 500) +
    scale_fill_manual(values = col, 
                      labels=c("afd"="AfD", "cdu"="CDU", "fdp"="FDP", "gruene" = "Die Grünen", "linke"="DIE LINKE", "spd"="SPD")) +
    xlab("") +
    ylab("Anzahl der Tweets") +
    scale_x_datetime(breaks = date_breaks("24 hour"), labels=date_format("%d/%m"))
```
...
oeffis %>%
  ...
  xlab("") +
  ylab("%Anteil der Tweets") +
  geom_col(fill = col[c(1,2,3,5,6)], alpha = .8) +
    scale_x_discrete(labels=c("afd"="AfD", "cdu"="CDU", "fdp"="FDP", "gruene" = "Die Grünen", "linke"="DIE LINKE", "spd"="SPD"))
```
...
gruene <- user[which(user$partei=="gruene"),]
linke <- user[which(user$partei=="linke"),]
...
gruene <- privat[which(privat$partei=="gruene"),]
linke <- privat[which(privat$partei=="linke"),]
...
#gruene <- oeffis[which(oeffis$partei=="gruene"),]
linke <- oeffis[which(oeffis$partei=="linke"),]
...
# Step 3: Plot comparison wordcloud ####
col <- c("black","red2", "gold","darkorchid2","deepskyblue1")
...
plot_tweets %>% 
  ...
  geom_col() +
  scale_fill_manual(values=col,
                    labels=c("afd"="AfD", "cdu"="CDU", "fdp"="FDP", "gruene" = "Die Grünen", "spd"="SPD")) +
  labs(x = NULL, y = "tf-idf") +
  coord_flip()
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
col <- c("deepskyblue1", "black","darkorchid2","red2")
...
plot_tweets %>% 
  ...
  geom_col() +
  scale_fill_manual(values=col,
                    labels=c("afd"="AfD", "cdu"="CDU", "linke"="DIE LINKE", "spd"="SPD")) +
  labs(x = NULL, y = "tf-idf") +
  coord_flip()
```
...
col <- c("deepskyblue1", "black", "gold","darkorchid2","red2","limegreen")
...
col <- c("deepskyblue1", "black", "gold","red2")
