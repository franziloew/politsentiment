...
```{r, include=FALSE}
tweets %>%
  filter(!grepl("funeralhall", text_cleaned)) -> tweets
```
...
orig %>%
  ...
  geom_col(fill = c("darkorchid2","gold", "red2","limegreen","black","deepskyblue1"), alpha = .8) +
  coord_flip()
```
...
news %>%
  ...
  xlab("") +
  ylab("%Anteil der Tweets") +
  geom_col(fill = c("darkorchid2","gold","limegreen", "red2", "deepskyblue1", "black"), alpha = .8) +
  coord_flip()
```
...
# Step 3: Plot comparison wordcloud ####
comparison.cloud(tdm, random.order=FALSE, colors = c("black","red2", "gold","limegreen" ,"deepskyblue1"),
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ignore <- c("cdu","bündnis", "spd","fdp","gruene","grüne","grünen", "linke","dielinke","afd","gen","merkel","schulz","csu","union","unions")
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ignore <- c("cdu","bündnis", "spd","fdp","gruene","grüne","grünen", "linke","dielinke","afd","gen","merkel","merkels","cducsu","schulz","csu","union","unions")
...
orig$text[grepl("pilz", orig$text_cleaned)]
```
...
plot_sentiments %>% 
  ...
  scale_fill_manual(values=c("deepskyblue1","black","limegreen", "gold","red2")) +
...
plot_sentiments %>% 
  ...
  ggplot(aes(reorder(word, count), count, fill = partei)) +
  geom_col(alpha = .9, show.legend = FALSE) +
  scale_fill_manual(values=c("deepskyblue1","black","limegreen", "darkorchid2","gold","red2")) +
...
news.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("black","darkorchid2","limegreen","deepskyblue1","gold", "red2"), alpha = .8) 
...
user.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("black","deepskyblue1","limegreen","gold","darkorchid2","red2"),alpha=.8) 
```
