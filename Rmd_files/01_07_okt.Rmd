---
title: "Politische Stimmung im Social Web nach der Bundestagswahl 2017"
...
ggplot(tweets, aes(date, fill = partei)) + 
    ...
    scale_x_datetime(breaks = date_breaks("24 hour"), labels=date_format("%a, %d.%m\n%H:%M", tz="CET"))
```
...
orig %>%
  ...
  geom_col(fill = c("limegreen","darkorchid2","gold", "red2","black","deepskyblue1"), alpha = .8) +
  coord_flip()
```
...
all <- c(cdu, spd, fdp, afd)

# Step 2: Corpus and term-docs_cleaned matrix ####
# create corpus
corp <- Corpus(VectorSource(all))
corp <- tm_map(corp, removeWords, c("cdu","spd","fdp","gruene","grüne","linke","dielinke","afd","gen","merkel","schulz","csu","union"))
...
# add column names
colnames(tdm) <- c("CDU","SPD","FDP","AfD")

# Step 3: Plot comparison wordcloud ####
comparison.cloud(tdm, random.order=FALSE, colors = c("black","red2", "gold","deepskyblue1"), scale=c(3,.5),
...
# create corpus
corp <- Corpus(VectorSource(all))
corp <- tm_map(corp, removeWords, c("cdu","spd","fdp","gruene","grüne","linke","dielinke","afd","merkel","schulz","csu","union"))
...
# Step 3: Plot comparison wordcloud ####
comparison.cloud(tdm, random.order=FALSE, colors = c("black","red2", "gold","deepskyblue1"),
                 scale=c(3,.5), title.size=1.2, max.words=100)
```

```{r, eval=FALSE, include=FALSE}
tweets$text[grepl("fedidwgugl",tweets$text_cleaned)]
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ignore <- c("merkel", "cdu","spd","fdp","gruene","gruenen")
...
plot_sentiments %>% 
  ...
  ggplot(aes(reorder(word, count), count, fill = partei)) +
  geom_col(alpha = .9) +
  scale_fill_manual(values=c("deepskyblue1","black","darkorchid2","gold","red2")) +
...
news.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("deepskyblue1","black","gold","darkorchid2", "red2"), alpha = .8) 
...
user.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("deepskyblue1","black","red2","darkorchid2","limegreen","gold"),alpha=.8) 
```
