...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ignore <- c("merkel","schulz","lindner", "union","linke","linken", "cdu","spd","fdp","grüne","grünen")
...
news.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("deepskyblue1","black","gold","darkorchid2","limegreen", "red2"), alpha = .8) 
...
user.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("black","deepskyblue1","limegreen","red2","darkorchid2","gold"),alpha=.8) 
```
