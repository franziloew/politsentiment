...
news %>%
  ...
  xlab("") +
  ylab("%Anteil der Tweets") +
  geom_col(fill = c("darkorchid2","limegreen", "gold","red2","deepskyblue1", "black"), alpha = .8) +
  coord_flip()
```
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
ignore <- c("cdu","spd","fdp","linke","linken","dielinke","gruene", "grüne","grünen", "merkel","schulz","lindner","union","wagenknecht")
...
news.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("black","red2","limegreen","gold","darkorchid2","deepskyblue1"), alpha = .8)
...
user.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("deepskyblue1","black","darkorchid2","limegreen","red2", "gold"),alpha=.8) 
```
