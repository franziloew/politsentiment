...
## Wie prominent wurde das Thema im betrachteten Zeitraum besprochen?
...
digi %>%
  ...
  ylab("%Anteil der Tweets") +
  #geom_col() +
  geom_col(fill = c("limegreen", "darkorchid2", "deepskyblue1","black","red2","gold"), alpha = .8) +
  coord_flip()
```
...
Da sowohl Bündis90/Die Grünen, als auch DIE LINKE nicht viele Tweets in zusammenhang mit "Digitalisierung" aufweisen, haben die Wordclouds zu diesen beiden Parteien eher wenig Aussagekraft. Der Vollständigkeit halber hier trotzdem aufgeführt:
...
# add column names
colnames(tdm) <- c("DIE LINKE", "Die Grünen")

# Step 3: Plot comparison wordcloud ####
comparison.cloud(tdm, random.order=FALSE, colors = c("darkorchid2", "limegreen"), scale=c(3,.5),
                 title.size=1.5, max.words=100)
```
...
plot_tweets %>% 
  ...
  geom_col(alpha=.9) +
  scale_fill_manual(name="", values=c("deepskyblue1","black","gold","red2")) +
