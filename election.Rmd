...
Wir betrachten einen Zeitraum von 8h (Sonntag, 24.09.2017, 16Uhr - 24Uhr)
...
tweets1 <- tweets[which(tweets$day == 24),]
tweets1 <- with(tweets1, tweets1[hour(date) >= 16 & hour(date) <= 23, ])
...
tweets1$isretweet <- ifelse(grepl('^RT', tweets1$text),TRUE,FALSE)
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
tweets1$retweet_count <- as.numeric(tweets1$retweet_count)
...
news.token %>% 
  ...
  ggplot(aes(reorder(partei,Sentimentwert), Sentimentwert)) +
          xlab("") +
           geom_col(fill = c("red2","black","gold","darkorchid2","deepskyblue1"), alpha = .8)
